...
pkgs <- c("shinysurveys", "shinyland", "shinymarkdown", "shinyrandomize")

shinyresearch_attach <- function() {
  # Create `to_load` which is a character vector of all shinyresearch
  ...
  # shinyresearch with the package version on the right-hand side
  load_header <- cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("shinyresearch ", package_version("shinyresearch"))
  )

  # Return a character string containing the package version for each of shinyresearch's constituents
...
shinyresearch_detach <- function() {
  pak <- paste0("package:", c(pkgs, "shinyresearch"))
  lapply(pak[pak %in% search()], detach, character.only = TRUE)
  invisible()
}

#' List all packages imported by shinyresearch
#'
#' @export
#'
#' @examples
#' shinyresearch_packages()
shinyresearch_packages <- function() {
  # get all imports from shinyresearch's package description file
  raw <- utils::packageDescription("shinyresearch")$Imports
